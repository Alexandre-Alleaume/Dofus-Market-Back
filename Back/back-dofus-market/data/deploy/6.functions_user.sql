...
/* SELECT ONE user by its email */
CREATE FUNCTION administration.get_user_by_email(email text) RETURNS administration.user AS $$
SELECT
  *
FROM
  administration.user
WHERE
  administration.user.email = get_user_by_email.email;

$$ LANGUAGE sql SECURITY DEFINER;

/* UPDATE A user */
CREATE
OR REPLACE FUNCTION administration.update_user(IN json_data jsonb) RETURNS text LANGUAGE 'plpgsql' VOLATILE SECURITY DEFINER PARALLEL UNSAFE COST 100 AS $ BODY $ DECLARE id integer;
...
BEGIN -- Extract the ticket ID from the JSON object.
...
SELECT
  *
FROM
  jsonb_each(json_data) LOOP IF key <> 'id' THEN IF value :: text <> 'null' THEN sql_statement := sql_statement || key || '= ' || value || ', ';
...
IF length(sql_statement) > 0 THEN sql_statement := left(sql_statement, length(sql_statement) - 2);
