...
AS $BODY$
DECLARE
    sql_statement text := 'INSERT INTO hdv (';
...
CREATE FUNCTION web.get_all_hdv() RETURNS SETOF web.hdv AS $$
SELECT
  *
FROM
  web.hdv;
...
CREATE FUNCTION web.get_hdv(hdv_id int) RETURNS web.hdv AS $$
SELECT
  *
FROM
  web.hdv
...
/* SELECT HDV BY USER */
CREATE FUNCTION web.get_user_hdv(user_id int) RETURNS web.hdv AS $$
SELECT
  *
FROM
  web.hdv
WHERE
  web.hdv.user_id = user_id;
